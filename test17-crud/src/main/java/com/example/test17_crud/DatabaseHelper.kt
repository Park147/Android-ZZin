...
// https://jw0652.tistory.com/590 와 바뀐점.
// 1.자바 코드를 코틀린 코드로 바꿈.
// 2. 뷰 객체 선택 방법이 findViewById를 ViewBinding으로 변경
...
// 4번째 매개변수 , 현재 사용하는 버전을 의미하고,
    ...
    // 이 클래스 사용하면, 지정된 데이터베이스 파일에, 테이블이 새로 생성된다.
    // 생성된 물리 경로 주소
    // 에물레이터 탐색기 data - data - com.example.test17_crud
    // -> database, 파일명으로 있음.

    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        // 위에 SqLiteOpenHelper
        //      (context, DATABASE_NAME, null, 1)
        // 4번째 매개 변수가 변경이 될때마다, 실행이 되는 함수.
    ...
    fun insertData(name: String?, phone: String?, address: String?): Boolean {
        // 디비를 사용시 쓰기, 수정, 삭제 -> writableDatabase 사용함.
        val db = this.writableDatabase
        // exexSQL - > 대신에 ContentValues() 를 이용하면
        // SQL 문장 X, 바로 메소드에 값만 인자로 넣어서 이용하면, 쉽게 insert를 구현가능.
