...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //test , MyApplication 에서 만든 형으로 Downcasting 하여, 해당 객체 생성.
        val networkService = (applicationContext as MyApplication).networkService

        //INetworkService 에 정의된 추상 함수이고, MyApplication 클래스에 붙였다.
        // 예)  doGetUserList(2)
    ...
    //Callback 인터페이스를 구현 특정 표현식 쓸 때, 꼭 재정의 해야하는 함수 2개
                //1. onResponse : 서버로부터 응답 성공시, 수행하는 함수
                //2.  onFailure : 실패 했을 대, 수행하는 함수
                override fun onResponse(
                    call: Call<UserListModel>,
                    response: Response<UserListModel>
                ) {
        // 정상적으로 서버로부터 데이터 받았다.
        //실제로 데이터 받고나서, response.body 곳에 data가 있음.
        val userList = response.body()

        // 받아온 데이터를 콘솔에 출력.
        // 실제 작업은 뷰에 바인딩.
        // 리사이클러 뷰를 작업.
        Log.d("psh","userList(response.body)의 값: ${userList}")
                    TODO("Not yet implemented")
                }
